...
func GenerateToken(userID int, ttl time.Duration, scope string) (*Token, error) { // ttl - time to life
	token := &Token{
		UserID: int64(userID),
...
func (m *DBModel) InsertToken(t *Token, u User) error {
	...
	// insert new token
	query = `insert into tokens (user_id, name, email, token_hash, expiry, created_at, updated_at)
			values (?, ?, ?, ?, ?, ?, ?)`
