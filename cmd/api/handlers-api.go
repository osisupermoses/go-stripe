...
import (
	...
	"github.com/osisupermoses/go-stripe/internal/validator"
...
// GetPaymentIntent gets payment intent object from stripe and sends it to client
...
// GetWidgetByID gets widgets by its ID (gotten from request URL)
...
// CreateCustomerAndsubscribeToPlan creates a customer and subscribes customer to a subscription plan
func (app *application) CreateCustomerAndSubscribeToPlan(w http.ResponseWriter, r *http.Request) {
	...
	// validate received data (do this for all the field validation)
	v := validator.New()
	...
	if okay {
		...
		txn := models.Transaction{
			...
			PaymentIntent:       subscription.ID, // here it's a subscription ID & not a payment intent
		...
		inv := Invoice{
			ID:        orderID,
			Amount:    2000, // shouldn't be hardcoded in prod
			Product:   "Bronze Plan monthly subscription", // normally we should do this dynamically, passing in a list of plans. But in our case, it's just one plan.
...
// CreateAuthToken creates authentication token
...
// authenticateToken authenticates API authorization token
...
// VirtualTerminalPaymentSucceeded reads request & saves transaction to db & sends receipt response back to client
func (app *application) VirtualTerminalPaymentSucceeded(w http.ResponseWriter, r *http.Request) {
	...
	// populate data that are absent from `txnData`` posted data
...
// SendPasswordResetEmail sends password reset email out
...
// AllSales renders the all subscriptions page
func (app *application) AllSales(w http.ResponseWriter, r *http.Request) {
	...
	allSales, lastPage, totalRecords, err := app.DB.GetAllOrdersPaginated(0, payload.PageSize, payload.CurrentPage)
	if err != nil {
		app.badRequest(w, err)
		return
	}
...
func (app *application) AllSubscriptions(w http.ResponseWriter, r *http.Request) {
	...
	allSubscriptions, lastPage, totalRecords, err := app.DB.GetAllOrdersPaginated(1, payload.PageSize, payload.CurrentPage)
	if err != nil {
		app.badRequest(w, err)
		return
	}
...
// GetSale renders the sales page
...
func (app *application) RefundCharge(w http.ResponseWriter, r *http.Request) {
	...
	// validate specified amount with amount coming from the db (do it yourself!)
...
func (app *application) CancelSubscription(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal json into object `subToCancel`
	...
	// update status to `CANCELLED (3)` in db
...
// AllUsers gets all registered users
...
// OneUser retrieves a registered user
...
// EditUser edits a particular user info and inserts a new user
func (app *application) EditUser(w http.ResponseWriter, r *http.Request) {
	...
	if userID > 0 {
		...
		msg = "User records modified successfully"
	} else {
		// inser a new user
		newHash, err := bcrypt.GenerateFromPassword([]byte(user.Password), 12)
		if err != nil {
